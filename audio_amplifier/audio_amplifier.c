...
int audio_extn_tfa98xx_start_feedback(void *adev, uint32_t snd_device) {
...
void audio_extn_tfa98xx_stop_feedback(void *adev, uint32_t snd_device) {
    tfa_dev->adev = (struct audio_device *)adev;
    if (!tfa_dev) {
...
static int amp_set_feedback(UNUSED amplifier_device_t* device, void *adev, uint32_t devices, bool enable) {
    tfa_dev->adev = (struct audio_device *)adev;
    if (enable) {
        audio_extn_tfa98xx_start_feedback(tfa_dev->adev, devices);
    } else {
        audio_extn_tfa98xx_stop_feedback(tfa_dev->adev, devices);
    }
    return 0;
}
