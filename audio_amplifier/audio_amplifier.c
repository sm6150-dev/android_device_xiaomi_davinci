...
int audio_extn_tfa98xx_start_feedback(uint32_t snd_device) {
...
void audio_extn_tfa98xx_stop_feedback(uint32_t snd_device) {
    if (!tfa_dev) {
...
static int amp_set_feedback(UNUSED amplifier_device_t* device, uint32_t devices, bool enable) {
    if (enable) {
        audio_extn_tfa98xx_start_feedback(devices);
    } else {
        audio_extn_tfa98xx_stop_feedback(devices);
    }
    return 0;
}
